import { BadgeCheck, BookOpenCheck, Briefcase, FileCode2, FileText, Lightbulb, Sparkles } from "lucide-react";

const quickTools = [
  { label: "Resume Builder", icon: Briefcase },
  { label: "Code Generator", icon: FileCode2 },
  { label: "Blog Writer", icon: FileText },
];
...
function RightToolsPanel({ suggestions, mode, learningTips, freelanceTasks, isDark }) {
  return (
    <aside className={`order-3 flex w-full flex-col gap-4 xl:order-none xl:w-[330px] ${isDark ? "text-slate-100" : "text-[#2E2E2E]"}`}>
      ...
      <section className={`rounded-2xl border p-4 shadow-sm ${isDark ? "border-slate-800 bg-slate-950" : "border-[#E7D8C4] bg-white"}`}>
            ...
            return (
              ...
              >
                <div className="mb-2 flex items-start justify-between gap-2">
                  ...
                  <span className={`rounded-full px-2 py-0.5 text-[10px] font-black ${difficultyStyle[task.difficulty] || "bg-slate-100 text-slate-700"}`}>
                    {task.difficulty}
                  </span>
                </div>
                ...
                <div className="mt-2 h-2 w-full overflow-hidden rounded-full bg-black/10">
                  <div className="h-full rounded-full bg-gradient-to-r from-[#7B1E1E] to-[#B64D2E]" style={{ width: `${task.progress}%` }} />
                </div>
        ...
        <div className={`mt-3 rounded-xl border px-3 py-2 text-xs ${isDark ? "border-slate-700 bg-slate-900 text-slate-300" : "border-[#E7D8C4] bg-[#FFF3E2] text-[#7B1E1E]"}`}>
