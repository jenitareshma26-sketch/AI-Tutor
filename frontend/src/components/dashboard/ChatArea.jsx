...
function ChatArea({ messages, isTyping, error, onCopy, onRegenerate, isDark }) {
        ...
        <div
          className={`w-full max-w-xl rounded-3xl border p-8 text-center shadow-sm ${
            isDark
              ...
              : "border-[#E7D8C4] bg-[#FFF9EF] text-[#4A0D14]"
          }`}
        >
          ...
          <p className={`mt-3 text-sm leading-7 ${isDark ? "text-slate-400" : "text-[#6E5A48]"}`}>
            Pick a mode, ask anything, and Tutorix AI will help you learn and ship practical output.
          </p>
        </div>
      </div>
    );
  ...
  return (
    <div className="flex-1 overflow-hidden">
      <div className="h-full space-y-4 overflow-y-auto scroll-smooth px-4 py-4 sm:px-6">
          ...
          return (
            <article key={message.id} className={`group flex ${isUser ? "justify-end" : "justify-start"}`}>
              ...
              >
                <div className="mb-2 flex items-center gap-2 text-xs font-semibold opacity-85">
                  <span
                    className={`inline-flex h-6 w-6 items-center justify-center rounded-full ${
                      isUser ? "bg-white/20" : isDark ? "bg-slate-800" : "bg-[#F8EFE0]"
                    }`}
                  >
                ...
                <ChatMessage content={message.content} isDark={isDark} isUser={isUser} />
                    ...
                    <button
                      ...
                      className={`inline-flex items-center gap-1 rounded-lg px-2.5 py-1 text-xs font-semibold transition ${
                        isDark
                          ...
                          : "bg-[#F8EFE0] text-[#6A2A2A] hover:bg-[#F1DFC6]"
                      }`}
                    >
                      <Copy size={12} /> Copy
                    </button>
        ...
        {isTyping && (
          <div className="flex justify-start">
            ...
            >
              <p className={`text-xs font-semibold ${isDark ? "text-slate-300" : "text-[#7B1E1E]"}`}>
                Tutorix AI is typing
              </p>
        ...
        {error && <div className="rounded-xl border border-red-300 bg-red-50 px-4 py-2 text-sm text-red-700">{error}</div>}
